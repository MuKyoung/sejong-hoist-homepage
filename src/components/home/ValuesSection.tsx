...
const VALUES = [
  {
    id: "wire", icon: <CheckCircle2 size={18} />, tab: "Wire Hoist",
    title: "중량물 인양의 핵심,\nWire Rope Hoist",
  ...
  {
    id: "chain", icon: <Wrench size={18} />, tab: "Chain Hoist",
    title: "소형·경량에 최적화된\nChain Hoist",
    body: "체인 방식으로 협소한 공간에도 설치할 수 있습니다. 0.25T부터 20T까지 지원하며 모노레일 시스템과 연계한 물류 환경에도 적합합니다.",
    metrics: [{ label: "최소 하중", value: "0.25T" }, { label: "설치 타입", value: "Low Head" }],
    img: "/images/sejong_2.png",
  },
  {
    id: "explosion", icon: <ShieldCheck size={18} />, tab: "방폭 Hoist",
  ...
  {
    id: "crane", icon: <Timer size={18} />, tab: "Crane",
    title: "현장 맞춤 설계로\n만드는 크레인",
...
export default function ValuesSection() {
  ...
  return (
    <section className={s.section} aria-label="제품 라인업">
      <div className={s.inner}>
        <div ref={ref} className={s.header}>
          <div>
            ...
            >
              <span className={s.tagLine} /> Product Line
            </motion.p>
          ...
          <motion.p className={s.headerDesc}
            ...
            transition={{ duration: 0.4, ease: E, delay: 0.15 }}
          >
            Wire·Chain·방폭 호이스트, 크레인.<br />
            현장 조건에 맞는 최적 제품을 설계·제작합니다.
          </motion.p>
        </div>
        ...
        >
          <Tabs.Root defaultValue="wire">
            <Tabs.List className={s.tabList} aria-label="제품 목록">
              {VALUES.map((v) => (
                ...
                <div className={s.tabPanel}>
                  <div className={s.tabTextArea}>
                    ...
                    <div className={s.metrics}>
                      {v.metrics.map((m, i) => (
                        <div key={i} className={s.metricChip}>
                          <span className={s.metricNum}>{m.value}</span>
                  ...
                  <div className={s.tabImage}>
                    <Image src={v.img} alt={v.tab} fill
                      style={{ objectFit: "cover" }}
                      sizes="(max-width: 768px) 100vw, (max-width: 1024px) 280px, 380px"
                    />
                  </div>
                </div>
              </Tabs.Content>
