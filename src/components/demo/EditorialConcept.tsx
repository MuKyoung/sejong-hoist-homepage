...
function Rise({
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 2.4, delay, ease: E }}
    >
      {children}
    </motion.div>
  );
    ...
    <div className="overflow-x-clip" style={{ background: "#fff", color: INK }}>
      ...
      >
        ...
        <div className="hidden md:block overflow-hidden"
          style={{
            ...
            transition: "height 1.1s ease, border-color 1.1s ease",
          }}>
          <div className="mx-auto flex items-center justify-between h-9 text-[12.5px]"
            style={{ maxWidth: 1400, paddingInline: "clamp(20px, 3.5vw, 48px)", color: solid ? "rgba(16,24,40,0.6)" : "rgba(255,255,255,0.78)", transition: "color 1.1s ease" }}>
            <div className="flex items-center gap-5">
              <span>{COMPANY.address}</span>
            </div>
            <div className="flex items-center gap-5">
              <a href={`tel:${COMPANY.tel.replace(/-/g, "")}`} className="hover:text-[#E8762C] transition-colors duration-1000">
                대표전화 {COMPANY.tel}
              </a>
              <span style={{ width: 1, height: 11, background: "rgba(255,255,255,0.25)" }} />
              <a href={`mailto:${COMPANY.email}`} className="hover:text-[#E8762C] transition-colors duration-1000">
                {COMPANY.email}
              </a>
              <span style={{ width: 1, height: 11, background: "rgba(255,255,255,0.25)" }} />
        ...
        <div ref={barRef} className="mx-auto grid grid-cols-[1fr_auto_1fr] items-stretch gap-6 h-14 lg:h-[64px]"
            ...
            {NAV.map((item, ni) => (
              ...
              >
                ...
                <span className="absolute left-5 right-5 xl:left-6 xl:right-6 bottom-0 h-[2.5px] origin-center scale-x-0 group-hover:scale-x-100"
                  style={{ background: "#E8762C", transition: "transform 1.2s cubic-bezier(0.16,1,0.3,1)" }} />
              </Link>
            ))}
          </nav>
          ...
          <div className="justify-self-end flex items-center gap-5">
            ...
            >
              <motion.span className="block h-[2px] w-6 rounded-full transition-colors duration-1000" style={{ background: fg, originX: "right" }}
                animate={{ rotate: menuOpen ? -45 : 0, y: menuOpen ? 8 : 0 }} transition={{ duration: 0.8, ease: E }} />
              <motion.span className="block h-[2px] w-4 rounded-full transition-colors duration-1000" style={{ background: fg }}
                animate={{ opacity: menuOpen ? 0 : 1 }} transition={{ duration: 0.5 }} />
              <motion.span className="block h-[2px] rounded-full transition-colors duration-1000" style={{ background: fg, originX: "right" }}
                animate={{ width: menuOpen ? 24 : 20, rotate: menuOpen ? 45 : 0, y: menuOpen ? -8 : 0 }} transition={{ duration: 0.8, ease: E }} />
            </button>
          </div>
        </div>
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-2">
                      {item.children.map((c, i) => (
                        ...
                        >
                          <Link href={c.href}
                            className="block whitespace-nowrap text-[13.5px] py-0.5 text-[#5E6E80] hover:text-[#E8762C] hover:translate-x-1 transition-all duration-1000">
                            {c.label}
                          </Link>
                        </motion.div>
                      ))}
            ...
            <div className="mx-auto grid sm:grid-cols-2 lg:grid-cols-5 gap-x-10 gap-y-12 pb-20"
              ...
              {NAV.map((item, i) => (
                <motion.div key={item.href}
                  ...
                  transition={{ delay: 0.2 + i * 0.16, duration: 1.8, ease: E }}
                >
                  ...
                  <div className="flex flex-col gap-3">
                    {item.children.map((c) => (
                      <Link key={c.href} href={c.href} onClick={() => setMenuOpen(false)}
                        className="text-[14px] transition-colors duration-600 hover:text-white"
      ...
      <section className="relative overflow-hidden" style={{ height: "min(86svh, 860px)", minHeight: 560 }}
        ...
        {SLIDES.map((s, i) => (
          <motion.div
            ...
            transition={{ opacity: { duration: 3.2, ease: "easeInOut" }, scale: { duration: 28, ease: "linear" } }}
          >
        ...
        <div className="absolute inset-0 flex items-center">
          <div className="w-full mx-auto" style={{ maxWidth: 1400, paddingInline: "clamp(20px, 3.5vw, 48px)" }}>
            <AnimatePresence mode="wait">
              <motion.div key={idx} exit={{ opacity: 0, y: -18, transition: { duration: 1, ease: E } }} style={{ maxWidth: 880 }}>
                {/* 명지대식 짧은 룰 */}
                <motion.span
                ...
                {SLIDES[idx].lines.map((line, li) => (
                  <div key={li} className="overflow-hidden">
                    <motion.h2
                      ...
                      transition={{ delay: 0.2 + li * 0.26, duration: 2.7, ease: E }}
                ...
                <motion.p
                  ...
                  transition={{ delay: 1.24, duration: 2.2, ease: E }}
          ...
          <div className="mx-auto flex items-center justify-between gap-6"
            ...
            <div className="flex items-center gap-2.5 text-white">
              <button type="button" onClick={() => go(-1)} aria-label="이전 슬라이드"
                className="w-11 h-11 rounded-full flex items-center justify-center border border-white/35 transition-all duration-1000 hover:bg-white/15 hover:border-white/70">
          ...
          <div className="grid lg:grid-cols-[1fr_1.1fr] gap-6 lg:gap-8 mt-14 lg:mt-20">
            ...
            <Rise x={-90} className="h-full">
              <Link href="/business" className="group relative flex flex-col justify-between overflow-hidden rounded-[28px] rounded-bl-none h-full min-h-[420px] lg:min-h-[540px] p-9 lg:p-12"
                ...
                >
                  <div className="w-full h-full rounded-full opacity-60 transition-transform duration-[2800ms] group-hover:scale-125"
                ...
                <span className="inline-flex items-center gap-2.5 h-12 px-6 self-start text-[13.5px] font-semibold transition-colors duration-1000 group-hover:bg-white group-hover:text-[#16273C]"
            ...
            <div className="flex flex-col gap-6 lg:gap-8">
              <Rise delay={0.12} x={90}>
                <Link href="/portfolio" className="group relative block overflow-hidden rounded-tr-[24px] min-h-[300px] lg:min-h-[340px]">
                  <Image src="/images/pf-gantry350.jpg" alt="350TON 겐트리 크레인" fill quality={85} sizes="(max-width: 1024px) 100vw, 720px"
                    className="object-cover group-hover:scale-[1.07]" style={{ transition: "transform 2.6s cubic-bezier(0.16,1,0.3,1)" }} />
                  ...
                  <div className="absolute left-0 right-0 bottom-0 p-8 lg:p-10 text-white">
                    ...
                    <span className="inline-flex items-center gap-2.5 h-11 px-5 mt-6 text-[13px] font-semibold backdrop-blur-[2px] transition-colors duration-1000 group-hover:bg-white group-hover:text-[#16273C]"
              ...
              <Rise delay={0.22} x={90}>
                <Link href="/technology" className="group relative overflow-hidden flex items-center justify-between gap-6 rounded-[20px] rounded-tr-none p-8 lg:p-10 min-h-[150px]"
                  ...
                  <span className="w-13 h-13 shrink-0 rounded-full flex items-center justify-center w-[52px] h-[52px] transition-all duration-1000 group-hover:bg-[#E8762C] group-hover:text-white group-hover:rotate-45"
          ...
          <div className="grid lg:grid-cols-[1.5fr_1fr] gap-x-16 gap-y-16">
            ...
            <Rise x={-90}>
              <div className="flex items-baseline justify-between gap-6 pb-5" style={{ borderBottom: `2px solid ${INK}` }}>
                <div className="flex items-baseline gap-6">
                  ...
                  <button type="button" onClick={() => setTab("notice")}
                    className="text-[clamp(22px,2.4vw,30px)] font-extrabold tracking-tight transition-colors duration-1200 hover:!text-[#E8762C]"
                ...
                <Link href={tab === "notice" ? "/support/notice" : "/portfolio"} aria-label="더보기"
                  className="text-[28px] font-light leading-none transition-all duration-1000 hover:rotate-90 hover:!text-[#E8762C]" style={{ color: INK }}>
                  +
                </Link>
              </div>
                ...
                >
                  {(tab === "notice"
                    ...
                    <li key={row.key} style={{ borderBottom: `1px solid ${HAIR}` }}>
                      <Link href={row.href} className="group flex items-center gap-5 py-[18px] transition-colors duration-1200 hover:bg-white"
                        ...
                        <span className="flex-1 min-w-0 text-[15px] font-medium truncate transition-colors duration-600 group-hover:text-[#E8762C]"
              ...
              <Link href={tab === "notice" ? "/support/notice" : "/portfolio"}
                className="group flex items-center justify-center gap-2.5 h-[58px] mt-8 text-[14px] font-bold transition-colors duration-1000 hover:bg-[#16273C] hover:text-white"
                style={{ border: `1px solid ${INK}` }}>
                {tab === "notice" ? "공지사항 전체 보기" : "시공사례 전체 보기"}
                <span className="transition-transform duration-1000 group-hover:translate-x-1"><NE /></span>
              </Link>
            </Rise>
            ...
            <Rise delay={0.15} x={90}>
              <div style={{ borderTop: `2px solid ${INK}` }}>
                ...
                <Link href="/technology#safety" className="group flex items-center justify-between py-5" style={{ borderBottom: `1px solid ${HAIR}` }}>
                  <span className="text-[15.5px] font-bold transition-colors duration-600 group-hover:text-[#E8762C]">안전관리 자세히 보기</span>
                  <span className="transition-transform duration-1000 group-hover:translate-x-1.5" style={{ color: ROYAL }}><NE /></span>
                </Link>
                <Link href="/technology#license" className="group flex items-center justify-between py-5" style={{ borderBottom: `1px solid ${HAIR}` }}>
                  <span className="text-[15.5px] font-bold transition-colors duration-600 group-hover:text-[#E8762C]">보유 자격 인력</span>
                  <span className="transition-transform duration-1000 group-hover:translate-x-1.5" style={{ color: ROYAL }}><NE /></span>
                </Link>
                <Link href="/support/inquiry" className="group flex items-center justify-between py-5" style={{ borderBottom: `1px solid ${HAIR}` }}>
                  <span className="text-[15.5px] font-bold transition-colors duration-600 group-hover:text-[#E8762C]">온라인 견적 문의</span>
                  <span className="transition-transform duration-1000 group-hover:translate-x-1.5" style={{ color: ROYAL }}><NE /></span>
                </Link>
              ...
              <div className="flex flex-col gap-3 mt-10">
                {QUICK_RAIL.map((q) => (
                  <Link key={q.label} href={q.href}
                    className="group flex items-center gap-4 h-[64px] px-6 text-white transition-colors duration-1000 hover:bg-[#E8762C] hover:!border-[#E8762C]"
                    style={{ border: "1px solid rgba(255,255,255,0.28)" }}>
                    {q.icon}
                    <span className="text-[15px] font-bold">{q.label}</span>
                    <span className="ml-auto transition-transform duration-1000 group-hover:translate-x-1.5"><NE /></span>
                  </Link>
                ))}
              </div>
            </Rise>
            ...
            <div className="grid sm:grid-cols-3 gap-5">
              {cards.map((item, i) => (
                <Rise key={item.slug} delay={0.12 + i * 0.14} x={80} className="h-full">
                  <Link href={`/portfolio/${item.slug}`} className="group flex flex-col h-full overflow-hidden rounded-[14px] rounded-bl-none backdrop-blur-[3px] transition-colors duration-1000 hover:bg-white"
                    style={{ background: "rgba(255,255,255,0.96)" }}>
                    <div className="relative overflow-hidden" style={{ aspectRatio: "4 / 3" }}>
                      <Image src={item.src} alt={item.title} fill sizes="(max-width: 640px) 100vw, 320px"
                        className="object-cover group-hover:scale-[1.08]" style={{ transition: "transform 2.6s cubic-bezier(0.16,1,0.3,1)" }} />
                    </div>
                    <div className="flex flex-col flex-1 p-6">
                      <span className="self-start text-[11.5px] font-bold px-2.5 py-1 mb-4" style={{ background: NAVY, color: "#fff" }}>
                        {item.capacity}
                      </span>
                      <h3 className="text-[16.5px] font-bold leading-[1.45] transition-colors duration-600 group-hover:text-[#E8762C]">
                        {item.title}
                      </h3>
                      <div className="mt-auto pt-5 flex items-center justify-between gap-4">
                        ...
                        <span className="shrink-0 transition-all duration-1000 group-hover:translate-x-1 group-hover:text-[#E8762C]"
          ...
          <Rise x={60} delay={0.1} className="flex-1">
            <div className="flex flex-wrap items-baseline gap-x-12 gap-y-3">
              {["LS ELECTRIC", "두산중공업", "현대위아"].map((n) => (
                <span key={n} className="font-extrabold tracking-tight transition-colors duration-1000 hover:text-[#E8762C]"
            ...
            <Rise delay={0.15} x={90}>
              <div className="flex flex-col sm:flex-row gap-4">
                <Link href="/support/inquiry"
                  className="flex items-center justify-center gap-2.5 h-[60px] px-10 rounded-full text-[15px] font-bold transition-colors duration-1000 hover:bg-[#F6E7D2]"
                  style={{ background: "#fff", color: NAVY }}>
                  온라인 견적 문의 <NE />
                </Link>
                <a href={`tel:${COMPANY.tel.replace(/-/g, "")}`}
                  className="flex items-center justify-center h-[60px] px-10 rounded-full text-[15px] font-bold text-white transition-colors duration-1000 hover:bg-white/10"
        ...
        <div className="mx-auto" style={{ maxWidth: 1400, paddingInline: "clamp(20px, 3.5vw, 48px)" }}>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 pb-9"
            ...
            <div className="flex flex-wrap gap-x-7 gap-y-2">
              {NAV.map((n) => (
                <Link key={n.href} href={n.href} className="text-[13.5px] font-semibold transition-colors duration-600 hover:text-white"
                  style={{ color: "rgba(255,255,255,0.55)" }}>
                  {n.label}
                </Link>
              ))}
      ...
      <div className="fixed right-4 sm:right-5 bottom-32 sm:bottom-24 z-[60] flex flex-col gap-3">
        <Link href="/support/inquiry" aria-label="견적 문의"
          className="relative w-14 h-14 sm:w-[64px] sm:h-[64px] rounded-full flex flex-col items-center justify-center gap-0.5 text-white text-[11px] font-bold shadow-[0_10px_30px_rgba(16,24,40,0.35)] transition-all duration-1000 hover:scale-110 hover:!bg-[#E8762C]"
          style={{ background: ROYAL }}>
          {/* 회전 대시 링 */}
          <motion.span
            aria-hidden
        ...
        <button type="button" aria-label="맨 위로"
          ...
          className="w-14 h-14 sm:w-[64px] sm:h-[64px] rounded-full flex flex-col items-center justify-center gap-1 text-[11px] font-bold bg-white shadow-[0_10px_30px_rgba(16,24,40,0.18)] transition-all duration-1000 hover:scale-110 hover:!bg-[#E8762C] hover:!text-white"
